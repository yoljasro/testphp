<?php
if(!isset($_GET['page'])){
    header("location:news-en.php?page=0");
    exit;
}else{
  ...
  if((intval($_GET['page'])>0)&&(intval($_GET['page'])*10+1>count($array_data))){
    header("location:news-en.php?page=".strval(intval(count($array_data)/10)));
    exit;
  }
}
?>
...
<body style="background-image:none !important;">
  ...
  <a href="index-en.php">  <i style=" font-size:40px;padding-left:20px" class="bi bi-house-fill"></i> </a>
  ...
  <section id="why-us" class="why-us" style="padding-top:100px;">
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-lg-12" style="margin-top:30px;">
          <a href="news-single-en.php?id=<?php echo ($_GET['page'])*10+$i?>">
            <div class="box" style="display:flex; flex-wrap:wrap;padding: 20px 20px;">
              ...
              <div class="col-lg-8 col-md-7 col-sm-12" style="margin-left:20px;">
                <h4 style="font-family: 'Lobster', cursive;"><?php echo $item['titleen']?></h4>
    ...
    <div style="text-align:center;padding-top:30px;font-size:25px;">
      <p class="noselect"><?php if(intval($GET['page'])>0){
        echo "<a href=\"news-en.php?page=".strval(intval($GET['page'])-1)."\"><i class=\"bi bi-arrow-left\"></i></a>";
      }?> <?php for($t=1;$t<=intval((count($array_data)-1)/10)+1;$t++){
        echo "<a href=\"news-en.php?page=".strval($t-1)."\">".strval($t)."</a> ";}?>   <?php 
      if($_GET['page']<intval((count($array_data)-1)/10)){
        echo "<a href=\"news-en.php?page=".strval(intval($GET['page'])+1)."\"><i class=\"bi bi-arrow-right\"></i></a>";}?></p>
      </div>
